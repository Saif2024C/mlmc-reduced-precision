...
// benchmark measures.)
//
// xf/xf_c are __m256h -- 16 lanes, matching fp32's RNG width, so both kernels
// retire equal paths per iteration and paths/sec is directly comparable.
// -------------------------------------------------------------------------
...
#pragma omp parallel for schedule(static) reduction(+:sink)
    ...
    for (long long i0 = 0; i0 < N; i0 += 16) {
        int nb = (int)std::min((long long)16, N - i0);
        avx_rng g; rng_seed(g, (unsigned)((i0 >> 4) * 131) + 1u);
        ...
        __m256h dw[5];
        // Correlated Brownian increments dW_i = sqrt(h) (L Z)_i, drawn and
        // Cholesky-combined natively in fp16 (norminv_fp16, not a downcast)
        B::draw_corr16(g, n16, (_Float16)sf_f, Lc, dw);

        __m256h incr[5], sigx[5];
        // Milstein INCREMENT only (not applied):
        //   dS_i = r S_i h + sigma_i S_i dW_i + (1/2) sigma_i^2 S_i (dW_i^2 - h)
        B::incr16(xf, dw, vHf16, R, SIG, incr, sigx);
        // Apply S_i <- S_i + dS_i by compensated summation, carrying c_i
        for (int i = 0; i < 5; ++i) opt::kahan_accum16(xf[i], xf_c[i], incr[i]);
        // Weighted basket value at maturity  B = sum_i w_i S_i(T)
        __m256h bf = B::value16(xf, W);

        alignas(64) _Float16 Ba[16];
        _mm256_store_ph(Ba, bf);
        double li = 0.0;
        for (int j = 0; j < nb; ++j) {
...
int main(int argc, char **argv)
{
    ...
    N = (N + 15) & ~((long long)15);          // round up to a whole 16-lane batch
