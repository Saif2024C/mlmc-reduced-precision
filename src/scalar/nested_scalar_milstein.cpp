/*
 * Nested MLMC for scalar Asian and Lookback calls — Milstein fp32/fp64 unified template.
 *
 ...
 *     - Both payoffs (option=1 Asian, option=2 Lookback) with l_star unused
 *
 ...
 *     - Per-option cutoffs: Asian l_star=5, Lookback l_star=6
 *
 ...
 *   Output files: nested_scalar_standard_{1,2}.txt, nested_scalar_adaptive_{1,2}.txt
 *     option 1 = Asian call,  option 2 = Lookback call
 *
 ...
 *   MPLBACKEND=Agg python3 python/nested_mlmc_plot_python.py outputs/nested_scalar_standard_2
 *   MPLBACKEND=Agg python3 python/nested_mlmc_plot_python.py outputs/nested_scalar_adaptive_1
 *   MPLBACKEND=Agg python3 python/nested_mlmc_plot_python.py outputs/nested_scalar_adaptive_2
 *   Output plots: outputs/nested/scalar/standard/nested_scalar_{standard,adaptive}_{1,2}_convergence.png
 *
 ...
 *     for opt in 1 2; do \
 *       MPLBACKEND=Agg python3 python/nested_mlmc_plot_python.py outputs/nested_scalar_${mode}_${opt}; \
 *     done; \
 *   done && \
...
static int  l_star = 0;
static const int l_star_by_option[] = {0, 5, 6};  // index by option 1/2
 ...
 *     l_star set per option: Asian=5, Lookback=6.
 *
 ...
 *   Asian: running sum via trapezoidal rule; lookback via exponential RV bridge
 *
 * =================================================================== */
void nested_scalar_l(int l, int N, double *sums)
{
    ...
    if (adaptive_mode && k >= l_star && l % 2 == 0) {
        for (int np = 0; np < N; ++np) {
            ...
            if (l == 0) {
                ...
                double Mf  = std::fmin(Xf0, 0.5*(Xf0+Xf - std::sqrt((Xf-Xf0)*(Xf-Xf0)
                                                                    - 2.0*hf_d*vf*vf*Lrv)));
                double Pf  = (option==1) ? std::fmax(0.0, Af-K_d) : (Xf-Mf);
                dP = disc_d * Pf; Pfv = dP;

            } else {
                double Xf=K_d, Xc=K_d;
                double Af=0.5*hf_d*K_d, Ac=0.5*hc_d*K_d;
                double Mf=K_d, Mc=K_d;

                for (int n = 0; n < nc; ++n) {
                    double dW0  = sqrtf(hf_d)*next_normal(), dW1 = sqrtf(hf_d)*next_normal();
                    double Lrv0 = -next_exponential(),       Lrv1= -next_exponential();
                    ...
                    Af += hf_d*Xf+vf0*dI0;
                    Mf  = std::fmin(Mf,0.5*(Xf0a+Xf-std::sqrt((Xf-Xf0a)*(Xf-Xf0a)-2.0*hf_d*vf0*vf0*Lrv0)));

                    double Xf0b=Xf, vf1=sig_d*Xf0b;
                    Xf  = Xf0b+r_d*Xf0b*hf_d+sig_d*Xf0b*dW1+0.5*sig_d*sig_d*Xf0b*(dW1*dW1-hf_d);
                    Af += hf_d*Xf+vf1*dI1;
                    Mf  = std::fmin(Mf,0.5*(Xf0b+Xf-std::sqrt((Xf-Xf0b)*(Xf-Xf0b)-2.0*hf_d*vf1*vf1*Lrv1)));
                    ...
                    double Xc1=0.5*(Xc0+Xc+vc*ddW);
                    Mc  = std::fmin(Mc,0.5*(Xc0+Xc1-std::sqrt((Xc1-Xc0)*(Xc1-Xc0)-2.0*hf_d*vc*vc*Lrv0)));
                    Mc  = std::fmin(Mc,0.5*(Xc1+Xc -std::sqrt((Xc-Xc1) *(Xc-Xc1) -2.0*hf_d*vc*vc*Lrv1)));
                }
                Af -= 0.5*hf_d*Xf; Ac -= 0.5*hc_d*Xc;
                double Pf=(option==1)?std::fmax(0.0, Af-K_d):(Xf-Mf);
                double Pc=(option==1)?std::fmax(0.0, Ac-K_d):(Xc-Mc);
    ...
    for (int np = 0; np < N; ++np) {
        ...
        // ================================================================
            ...
            float Pf_f  = (option == 1) ? std::fmaxf(0.0f, Af_f - K_f) : (Xf_f - Mf_f);
            ...
            double Mf_d  = std::fmin(Xf0_d,
                               0.5*(Xf0_d + Xf_d
                                    - std::sqrt((Xf_d-Xf0_d)*(Xf_d-Xf0_d)
                                                - 2.0*hf_d*vf_d*vf_d*Lrv_d)));

            double Pf_d  = (option == 1) ? std::fmax(0.0, Af_d - K_d) : (Xf_d - Mf_d);
        ...
        // ================================================================
            ...
            for (int n = 0; n < nc; ++n) {
                ...
                float Lrv0 = -next_exponential();
                float Lrv1 = -next_exponential();
                ...
                Mf  = std::fminf(Mf,
                          0.5f*(Xf0a + Xf
                                - sqrtf((Xf-Xf0a)*(Xf-Xf0a)
                                        - 2.0f*hf_f*vf0*vf0*Lrv0)));
                ...
                Mf  = std::fminf(Mf,
                          0.5f*(Xf0b + Xf
                                - sqrtf((Xf-Xf0b)*(Xf-Xf0b)
                                        - 2.0f*hf_f*vf1*vf1*Lrv1)));
                ...
                // Coarse lookback: reconstruct midpoint Xc1
                float Xc1  = 0.5f * (Xc0 + Xc + vc * ddW);
                Mc  = std::fminf(Mc,
                          0.5f*(Xc0 + Xc1
                                - sqrtf((Xc1-Xc0)*(Xc1-Xc0)
                                        - 2.0f*hf_f*vc*vc*Lrv0)));
                Mc  = std::fminf(Mc,
                          0.5f*(Xc1 + Xc
                                - sqrtf((Xc-Xc1)*(Xc-Xc1)
                                        - 2.0f*hf_f*vc*vc*Lrv1)));
            }
        ...
        // ================================================================
            ...
            for (int n = 0; n < nc; ++n) {
                ...
                double Lrv0_d = Lrv0_f, Lrv1_d = Lrv1_f;
                ...
                Mf_d  = std::fmin(Mf_d,
                            0.5*(Xf0a_d + Xf_d
                                 - std::sqrt((Xf_d-Xf0a_d)*(Xf_d-Xf0a_d)
                                             - 2.0*hf_d*vf0_d*vf0_d*Lrv0_d)));
                ...
                Mf_d  = std::fmin(Mf_d,
                            0.5*(Xf0b_d + Xf_d
                                 - std::sqrt((Xf_d-Xf0b_d)*(Xf_d-Xf0b_d)
                                             - 2.0*hf_d*vf1_d*vf1_d*Lrv1_d)));
                ...
                double Xc1_d  = 0.5 * (Xc0_d + Xc_d + vc_d * ddW_d);
                Mc_d  = std::fmin(Mc_d,
                            0.5*(Xc0_d + Xc1_d
                                 - std::sqrt((Xc1_d-Xc0_d)*(Xc1_d-Xc0_d)
                                             - 2.0*hf_d*vc_d*vc_d*Lrv0_d)));
                Mc_d  = std::fmin(Mc_d,
                            0.5*(Xc1_d + Xc_d
                                 - std::sqrt((Xc_d-Xc1_d)*(Xc_d-Xc1_d)
                                             - 2.0*hf_d*vc_d*vc_d*Lrv1_d)));
            }
            ...
            float  Pff = (option==1)?std::fmaxf(0.0f,Af_f-K_f):(Xf_f-Mf_f);
            float  Pcf = (option==1)?std::fmaxf(0.0f,Ac_f-K_f):(Xc_f-Mc_f);
            double Pfd = (option==1)?std::fmax(0.0, Af_d-K_d) :(Xf_d-Mf_d);
            double Pcd = (option==1)?std::fmax(0.0, Ac_d-K_d) :(Xc_d-Mc_d);
